    ...
    <div class="col-md-10 content-wrapper">
        ...
        <div class="widget widget-table">
            ...
            <div class="dgContainer widget-content table-responsive">
                ...
                <div class="row">
                    <div class="col-md-6 dgButtons"><a class="btn btn-primary btn-sm" href="{{ route('get.crm.calendar.create') }}">
                            <i class="fa fa-plus">
                            </i> Thêm sự kiện</a></div>
                    ...
                    <tbody>
                        @foreach ($calendars as $calendar)
                            <tr class="calendarStt1">
                                ...
                                <td>
                                    {{ $calendar->begin_date_time }} <i class="fa fa-arrow-right"></i> {{ $calendar->end_date_time }}
                                </td>
                                <td>
                                ...
                                <td class="colControls">
                                    <a class="fa fa-edit icon" href="{{ route('get.crm.calendar.edit',$calendar->id) }}">
                                    </a>&nbsp;
                                    <a class="fa fa-trash deleteCalendar icon" 
                                    onclick="event.preventDefault();
                                    document.getElementById('modalbox').style.display='block'; 
                                    deletedId = 'destroy'+{{ $calendar->id }}"></a>
                                    <form action="{{ route('post.crm.calendar.destroy') }}" method="POST" style="display: none">
                                        @csrf
                                        <input type="submit" style="display: none" name="id" id="{{ 'destroy'.$calendar->id }}" value="{{ $calendar->id }}">
                                    </form>
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <p>Chắc chắn muốn xóa sự kiện này</p>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-primary" onclick="document.getElementById(deletedId).click()">Delete</button>
                  <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="document.getElementById('modalbox').style.display='none'">
                      Close
                </button>
                </div>
              </div>
