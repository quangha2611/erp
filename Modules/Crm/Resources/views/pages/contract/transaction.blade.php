        ...
        <table id="dgContractPayment" class="table table table-hover">
            ...
            <tbody>
                @php
                    $totalAllMoney        = 0;
                    $totalAllMoneyDone    = 0;
                    $totalAllMoneyNotDone = 0;
                @endphp
                @foreach ($contractTransactions as $transaction)
                    @php
                        $totalAllMoney += intval($transaction->contract['totalValue']);
                        if ($transaction->is_checked == 0)
                            $totalAllMoneyNotDone += intval($transaction->contract['totalValue']);
                        else
                            $totalAllMoneyDone += intval($transaction->contract['totalValue']);
                    @endphp
                    <tr class="">
                        <td>
                            <a class="text-primary" href="{{ route('get.crm.contract.showtransaction', ['id' => $transaction->id]) }}">
                                {{ $transaction->id }}
                            </a>
                        </td>
                        <td>
                            <a style="color: #F73838;" href="{{ route('get.crm.contract.show', ['id' => $transaction->contract->id]) }}">{{ $transaction->contract->id }}</a>
                        </td>
                        <td>
                            <a href="{{ route('get.crm.customer.detail',['id' => $transaction->contract->customer->id ]) }}">
                                {{ $transaction->contract->customer->name }}
                            </a>
                        </td>
                        <td class="colControls"><b title="Phiếu thu">Thu</b></td>
                        <td class="colNb"><a style="color: #F73838;"
                                href="https://erp.nhanh.vn/crm/contract/viewtransaction?id=76816">{{ number_format($transaction->contract['totalValue']) }}</a>
                        </td>
                        <td class="colControls">
                            @if ($transaction->type_id == 1) 
                                <i class="fa fa-bank" data-toggle="tooltip" title="Chuyển khoản"></i>
                            @else
                                <i class="fa fa-money" data-toggle="tooltip" title="Tiền mặt"></i>
                            @endif
                        </td>
                        ...
                        <td class="col-align-center col-status">
                            @if ($transaction->is_checked == 0)
                                <a class=" color-orange" data-toggle="tooltip" title="" data-original-title="Bấm để duyệt">
                                    <b>Chưa duyệt</b>
                                </a>
                            @else 
                                <b data-toggle="tooltip" class="text-success" title="Đã duyệt">Đã duyệt</b>
                            @endif
                        </td>
                        @if ($transaction->checker_id != null)
                            <td>
                                <div data-toggle="tooltip" style="width:auto;"
                                    data-original-title="{{ $transaction->checker_time }}" class="">{{ $transaction->checker->name }}
                                </div>
                            </td>
                        @else
                            <td>
                ...
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td><b>{{ number_format($totalAllMoney) }}</b></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td>
                        <b data-toggle="tooltip" data-title="Tổng tiền đã duyệt" class="text-success">
                            {{ number_format($totalAllMoneyDone) }}
                        </b>
                        <br>
                        <b data-toggle="tooltip" data-title="Tổng tiền chưa duyệt">
                            {{ number_format($totalAllMoneyNotDone) }}
                        </b>
                    </td>
                    <td></td>
                    <td></td>
...
@section('js')
    ...
    <script type="text/javascript" src="{{ asset('js/crm/contract/transaction.js') }}"></script>
@endsection