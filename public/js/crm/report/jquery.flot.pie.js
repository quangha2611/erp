...
var canvasWidth=plot.getPlaceholder().width(),canvasHeight=plot.getPlaceholder().height(),legendWidth=target.children().filter(".legend").children().width()||0;ctx=newCtx;processed=false;maxRadius=Math.min(canvasWidth,canvasHeight/options.series.pie.tilt)/2;centerTop=canvasHeight/2+options.series.pie.offset.top;centerLeft=canvasWidth/2;if(options.series.pie.offset.left=="auto"){if(options.legend.position.match("w")){centerLeft+=legendWidth/2;}else{centerLeft-=legendWidth/2;}
...
var halfAngle=((startAngle+slice.angle)+startAngle)/2;var x=centerLeft+Math.round(Math.cos(halfAngle)*radius);var y=centerTop+Math.round(Math.sin(halfAngle)*radius)*options.series.pie.tilt;var html="<span class='pieLabel' id='pieLabel"+index+"' style='position:absolute;top:"+y+"px;left:"+x+"px;'>"+text+"</span>";target.append(html);var label=target.children("#pieLabel"+index);var label_w=label.width();var label_h=label.height();var labelTop=(y-label_h/2);var labelLeft=(x-label_w/2);var label_pos=[[labelLeft,labelLeft+label_w],[labelTop,labelTop+label_h]];var newradius=radius;var bCollision=false;var yix=10;do{for(var j=(labels.length-1);j>=0;j--)
{while(comparePositions(label_pos[0],labels[j][0])&&comparePositions(label_pos[1],labels[j][1]))
{newradius-=2;if(newradius<0.00){break;}
x=centerLeft+Math.round(Math.cos(halfAngle)*newradius);y=centerTop+Math.round(Math.sin(halfAngle)*newradius)*options.series.pie.tilt;labelTop=(y-label_h/2);labelLeft=(x-label_w/2);label_pos[0][0]=labelLeft;label_pos[0][1]=labelLeft+label_w;label_pos[1][0]=labelTop;label_pos[1][1]=labelTop+label_h;bCollision=true;}
if(bCollision)break;}
label.css("top",labelTop);label.css("left",labelLeft);if(bCollision)bCollision=false;else break;yix--;}while(yix>0);function comparePositions(p1,p2){var x1=p1[0]<p2[0]?p1:p2;var x2=p1[0]<p2[0]?p2:p1;return x1[1]>x2[0]||x1[0]===x2[0]?true:false;}
labels.push(label_pos);if(0-labelTop>0||0-labelLeft>0||canvasHeight-(labelTop+label.height())<0||canvasWidth-(labelLeft+label.width())<0){return false;}
